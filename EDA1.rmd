...
all_beer_ds %>% ggplot() +
  geom_point(aes(x = ABV, y = IBU, color = IBU), position = "jitter", alpha = .5) +
...
#all_beer_ds[, 5:6]
class <- knn(train[, 5:6], test, train$Style, k = 5, prob = T)
...
```{r}
# KNN Classification | FOR PRESENTATION ####
train2 = train[grepl("Ale", train$Style), ]

test2 = train[grepl("IPA", train$Style), ]

#all_beer_ds[, 5:6]
class2 <- knn(train2[, 5:6], test2[, 5:6], train2$Style, k = 5, prob = T)
