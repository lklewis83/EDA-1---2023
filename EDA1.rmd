...
# Probability of finding an IPA in a list of ALE beers
class2 <- knn(train2[, 4:5], test2[, 4:5], train2$Style, k = 5, prob = T)

class2

## Probability of finding an IPA similar to an ALE based off ABV or IBU
...
train_new = sample(1:dim(train2)[1], round(splitPerc * dim(train2)[1]))
ALL_Ale_Train = train2[train_new, ]
ALL_Ale_Test = train2[-train_new, ]

class4 <-knn(ALL_Ale_Train[, c(4:5)], ALL_Ale_Test[, c(4:5)], ALL_Ale_Train$Style, k = 5, prob = T)

confusionMatrix(table(ALL_Ale_Test[ , 6], class4))



class5 <- knn.cv(train2[, c(4:5)], train2$Style, k = 5, prob = T)
