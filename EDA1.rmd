...
TEst Plot | Bad Results - Wont Use
```{r}
# MIssing Data PLot 1 ####
g <- ggplot() +
...
# Add IBU for Styles that couldn't get a median from aggregate function ####
# Grab the index for missing IBU
for (m in 1:length(missing_IBU)) {
  
 
  missing_index_IBU[m] = which(grepl(missing_IBU[m], all_beer_ds$Style))
  
  missing_index_ale = which(all_beer_ds$Style == "American Wild Ale")
  
  missing_index_american = which(all_beer_ds$Style == "American Double")
  
  missing_index_a_IPA = which(all_beer_ds$Style == "American IPA")
}

# Update missing IBU with default Median IBU
for (n in 1:length(missing_IBU)) {
  
 
  all_beer_ds[missing_index_IBU[n], "IBU"] = no_style_IBU_cnt
 
}


for (a in 1:length(missing_index_ale)) {
  all_beer_ds[missing_index_ale[a], "IBU"] = no_style_IBU_cnt
}
...
## IBU LOOP MEDIAN ####
for (j in 1:length(style_IBU_cnt)) {
  print(j)
