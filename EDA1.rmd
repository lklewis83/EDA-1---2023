...
Ale_index <- which(grepl("Ale", all_beer_ds$Style))
...
# Update all American IPA beers with the following value
all_beer_ds$IBU[all_beer_ds$Style == "American IPA" & is.na(all_beer_ds$IBU)] <- 55

# ALE Replacement
for (a in 1:length(Ale_index)) {
  
  all_beer_ds[Ale_index[a], "IBU"] = 35
}  
...
for (s in 1:length(Stout_index)) {
  ...
  all_beer_ds[Stout_index[s], "IBU"] = no_style_IBU_cnt
} 
...
Max_ABV$Lat = 37.8393
Max_ABV$Long = 84.2700

Max_IBU <- head(Top_5_IBU, 1)
Max_IBU$Lat = 64.2008
Max_IBU$Long = 149.4937
...
plot_usmap(data = median_map, values = "ABV_Median") +
  geom_point( x = 37.8393, y = 84.2700, color = "black", size = 4, shape = 16) +
  labs(title = "Max ABV State: Kentucky") +
...
plot_usmap(data = median_map, values = "IBU_Median") +
  geom_point( x = 61.2176, y = 149.8997, color = "black", size = 4, shape = 16) +
  labs(title = "Max IBU State: Alaska") +
  scale_fill_gradient(low = "dark grey", high = "red") 
